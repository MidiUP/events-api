...
export class EventCreateValidator implements Validator {
  private readonly schema = yup.object().shape({
    ...
    availableTickets: yup.number().required('availableTickets is a required field').typeError('availableTickets shold be a number').min(0, 'availableTickets shold be greater or equals than 0'),
